...
const BackdropImage = styled.div`
  background: url(${(props) => props.imageUrl || "black"}) center center / cover
    no-repeat;
  width: 1024px;
  position: relative;
  top: auto;
...
const DetailMovie = ({ detail, loading }) => {
  const year = useMemo(
    () => (detail ? detail.release_date.split("-")[0] : ""),
    [detail]
  );
          ...
          <TopInfo>
            ...
            <PosterContainer>
              <Backdrop>
                <LeftBlur />
                <BackdropImage
                  imageUrl={`${process.env.REACT_APP_IMAGE_PREFIX}/${detail.backdrop_path}`}
                >
                  <LeftGradient />
                  <RightGradient />
                </BackdropImage>
            ...
            <Main>
              <Container>
                <PosterWrapper>
                  <Poster
                    src={`${process.env.REACT_APP_IMAGE_PREFIX}/${detail.poster_path}`}
                  />
                </PosterWrapper>
                <ContentWrapper>
                  {/* 기본정보 */}
                  ...
                  <AverageRate>
                    평균 ★{detail.vote_average} ({detail.vote_count}명)
                  </AverageRate>
                  <Actions>
                    <StarRate>
